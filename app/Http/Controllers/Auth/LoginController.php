...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if (Auth::attempt(['email' => $credentials['email'], 'password' => $credentials['password']], $request->filled('remember'))) {
            ...
            if ($user->role === User::ROLE_STAFF || $user->role === User::ROLE_ADMIN) {
                Auth::logout();

                $portal = $user->role === User::ROLE_ADMIN ? 'admin' : 'nhân viên';
                $route = $user->role === User::ROLE_ADMIN ? 'admin.login' : 'staff.login';

                return redirect()->route($route)
                    ->withErrors(['email' => "Tài khoản này thuộc phân hệ {$portal}. Vui lòng đăng nhập tại cổng {$portal}."])
